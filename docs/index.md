...
The package is especially useful when you want to move beyond manual
work in the Heurist web interface and begin building reproducible R
workflows for:
...
Most users will spend most of their time with the higher-level helpers
such as
...
[`heurist_get_record()`](https://center-for-archaeology-and-society.github.io/heuristR/reference/heurist_get_record.md),
...
The low-level wrappers are still available when you need to inspect or
debug the raw Heurist payloads.
...
file in the project root. Do not commit your own `.Renviron` file or any
file that contains live credentials to a git repository.
...
1.  [`heurist_session()`](https://center-for-archaeology-and-society.github.io/heuristR/reference/heurist_session.md)
    creates a client object with the base URL, database name, timeout,
    and cookie storage.
2.  [`heurist_login()`](https://center-for-archaeology-and-society.github.io/heuristR/reference/heurist_login.md)
    uses that client to authenticate and retain the returned session
    cookies.

That separation keeps configuration distinct from authentication and
makes it possible to construct a session object before logging in.
...
If you prefer a more compact mental model, you can think of this as
“create the client, then authenticate the client.”
...
The examples below are illustrative and may need to be adapted to your
own Heurist database structure, record types, field IDs, and
permissions.

## Inspect Database Structure

Before writing automation against a Heurist database, it is usually
helpful to inspect the available record types and field definitions.
...
- understanding how a project database models people, places, objects,
  or events
...
There are two common read patterns: fetch a known record by ID, or
...
If you want the raw endpoint response rather than the higher-level
parsed helper:
...
New records can be created with
[`heurist_create_record()`](https://center-for-archaeology-and-society.github.io/heuristR/reference/heurist_create_record.md).
For example, the following illustrative example creates a fictional
`Place` record:
...
The returned object is a `heurist_change`, which includes enough
information to support rollback later if needed.
...
payloads. `heuristR` avoids that by using a read-modify-write approach
in
[`heurist_patch_record()`](https://center-for-archaeology-and-society.github.io/heuristR/reference/heurist_patch_record.md).
...
This helper first fetches the existing record, merges your requested
change in R, and then submits a full replacement payload back to
Heurist.
...
If you need a full explicit replacement rather than a merge-style patch,
use
[`heurist_replace_record()`](https://center-for-archaeology-and-society.github.io/heuristR/reference/heurist_replace_record.md).
...
Heurist databases often use resource pointer fields to connect records.
For that case,
[`heurist_link_record()`](https://center-for-archaeology-and-society.github.io/heuristR/reference/heurist_link_record.md)
provides a clearer workflow than manually constructing all the detail
payloads yourself.
...
One of the most useful features of `heuristR` is that the high-level
write helpers return change objects that can be undone.
...
If you need raw access to the underlying Heurist controllers, the
package also provides low-level wrappers.
